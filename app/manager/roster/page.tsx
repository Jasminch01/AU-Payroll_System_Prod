...
export default function ManagerRosterPage() {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('manager-roster-realtime')
            .on(
                'postgres_changes',
                {
                    event: '*',
    ...
    const handleAddShift = (notify = false) => {
        ...
        if (currentRoster) {
            ...
            if (selectedDate < rosterStart || selectedDate > rosterEnd) {
                // Determine what the new period would be
                const dStart = new Date(rosterStart < rangeStart ? rosterStart : rangeStart);
                const dEnd = new Date(rosterEnd > rangeEnd ? rosterEnd : rangeEnd);
                const diffDays = Math.ceil((dEnd.getTime() - dStart.getTime()) / (1000 * 60 * 60 * 24)) + 1;

                let targetType = "monthly";
                if (diffDays <= 7) targetType = "weekly";
                else if (diffDays <= 14) targetType = "fortnightly";
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex flex-col items-end">
                                    ...
                                    <span className="text-[10px] text-[hsl(var(--muted-foreground))] uppercase tracking-wider font-medium mt-1">
                                        {statusSummary.published} of {statusSummary.total} shifts notified • {formatDurationHours(statusSummary.totalPublishedHours)} published
                                    </span>
                                </div>
                            </div>
                        ) : (
