...
export async function GET(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        const { data: employee, error } = await supabase
            .from('Employee')
            .select('employee_id, first_name, last_name, phone, email, dob, bank_details, bank_account_name, bank_bsb, bank_account_number, abn, tfn, emergency_contact_name, emergency_contact_phone, employment_type, role_title, pay_cycle, start_date, end_date, created_at, updated_at, business_id, user_id, status')
            .eq('employee_id', id)
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        if (authUser.role === 'owner' || authUser.role === 'employee') {
            allowedFields.push('bank_details', 'bank_account_name', 'bank_bsb', 'bank_account_number', 'abn', 'tfn');
        }
