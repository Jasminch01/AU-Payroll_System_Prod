...
export async function PUT(
    ...
    try {
        ...
        const { data: existingItems, error: existingError } = await supabase
            ...
            .eq('shift_id', id)
            .in('source_template_id', template_ids);

        if (existingError) return errorResponse(existingError.message, 400);

        if (existingItems && existingItems.length > 0) {
            const alreadyAttachedIds = Array.from(new Set(
                existingItems.map(item => item.source_template_id).filter(Boolean)
            ));
            if (alreadyAttachedIds.length > 0) {
                const { data: templatesData } = await supabase
                    .from('ChecklistTemplate')
                    .select('name')
                    .in('template_id', alreadyAttachedIds);
                
                if (templatesData && templatesData.length > 0) {
                    const names = templatesData.map(t => `"${t.name}"`).join(', ');
                    return errorResponse(`${names} already attached`, 400);
                }
                return errorResponse('Template already attached', 400);
            }
        }
